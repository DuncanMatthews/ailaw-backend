...
import { CaseComponent } from '@/components/cases/case-depth'
...
async function submitUserMessage(content: string) {
  ...
  let similarCases = currentState.currentCase

  // Check if the user is asking for a new case or if similarCases is empty
  if (!similarCases || content.includes('new')) {
    const embedding = await generateEmbedding(content)
    similarCases = await findSimilarCases(embedding)
    console.log(similarCases)

    // Update the currentCase in state
    aiState.update({
      ...currentState,
      currentCase: similarCases
    })
  }
  ...
  const result = await streamUI({
    ...
    system: `
    You are a legal assistant conversation bot, helping lawyers find relevant information from court cases to support their arguments. The case details are provided in the similarCases variable.

    Context:
    ${JSON.stringify(similarCases, null, 2)}

    The lawyer will provide a scenario or ask a question related to the case. Use the information from the provided case to assist the lawyer in finding relevant details, such as:
    ...
    - Ratio decidendi
    - Obiter dicta
    - Cited cases and their relevance
    - Cited legislation and its relevance

    If the lawyer asks about specific aspects of the case, provide the relevant information from the case details. If the requested information is not available in the provided case, respond with "I apologize, but the case details do not contain information about [the requested aspect]."

    Important: Only provide information that is present in the case details. Do not generate or assume any information that is not explicitly provided.

    If the lawyer asks a question that is not related to the provided case or requests an action that is not supported, politely inform them that you can only assist with information from the given case.

    Once you are done responding , call 
  `,
...
chatId: string
messages: Message[]
currentCase: any | null // Add this line
}
